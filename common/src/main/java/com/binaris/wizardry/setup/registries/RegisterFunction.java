...
///
/// @param <T> the type being registered (e.g. `Item`, `Block`)
@FunctionalInterface
public interface RegisterFunction<T> {
    ...
    ///
    /// @param registry the [Registry] to register into, may be `null` when the registry instance
    ///                 is not required by the caller
    /// @param id       the [ResourceLocation] to register the object under
