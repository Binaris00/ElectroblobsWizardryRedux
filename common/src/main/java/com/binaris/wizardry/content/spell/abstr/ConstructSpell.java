...
/// Base class for spells that spawn magical construct entities ([MagicConstructEntity]). Handles the creation,
...
/// as intended.
///
/// @param <T> The type of [MagicConstructEntity] this spell creates.
public class ConstructSpell<T extends MagicConstructEntity> extends Spell {
    ...
    ///
    /// If [#allowOverlap] is false, this method checks for existing constructs of the same type at the
    /// target position and prevents spawning if one exists.
    ///
    /// This method also calls [#addConstructExtras(CastContext, MagicConstructEntity, Direction)] to allow
