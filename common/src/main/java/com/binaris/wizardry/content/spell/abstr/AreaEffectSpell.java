...
///
/// Check [`Firestorm`][com.binaris.wizardry.content.spell.fire.Firestorm] and [`FontOfMana`][com.binaris.wizardry.content.spell.healing.FontOfMana]
...
public abstract class AreaEffectSpell extends Spell {
    ...
    ///
    /// The actual particle count is calculated as `particleDensity * π * radius²`, so higher values
    ...
    /// based on the [#targetAllies] setting, sorts them by distance from the origin, and then
    /// calls [#affectEntity(CastContext, Vec3, LivingEntity, int)] for each target.
    ///
    ...
    ///
    /// The number of particles is determined by `particleDensity * π * radius²`. Particles are spawned
    /// at random positions within the radius, distributed in a circle around the origin.
    ///
    /// This method calls [#spawnParticle(Level, double, double, double)] for each particle position.
    /// Override `spawnParticle` to define which particles to spawn.
    ///
    ...
    /// is called multiple times by [#spawnParticleEffect(CastContext, Vec3, double)] to create the full particle
    /// effect.
    ///
