...
public class RandomSpellFunction extends LootItemConditionalFunction {
    ...
    protected @NotNull ItemStack run(ItemStack stack, @NotNull LootContext context) {
        if (!(stack.getItem() instanceof SpellBookItem) && !(stack.getItem() instanceof ScrollItem)) EBLogger.warn("Applying the random_spell loot function to an item that isn't a spell book or scroll.");
        ...
        if (spell == Spells.NONE) EBLogger.warn("Tried to apply the random_spell loot function to an item, but not enabled spells matched the criteria specified. Substituting placeholder (metadata 0) item.");
    ...
    public static class Serializer extends LootItemConditionalFunction.Serializer<RandomSpellFunction> {
        ...
        public @NotNull RandomSpellFunction deserialize(JsonObject object, @NotNull JsonDeserializationContext deserializationContext, LootItemCondition @NotNull[] conditions) {
            ...
            if (object.has("spells")) {
                ...
                if(result.result().isPresent()) spells =  result.result().get().stream().map(Services.REGISTRY_UTIL::getSpell).collect(Collectors.toList());
            }
            ...
            if (object.has("tiers")) {
                ...
                if(result.result().isPresent()) tiers =  result.result().get().stream().map(Services.REGISTRY_UTIL::getTier).collect(Collectors.toList());
            }

            if (object.has("elements")) {
                DataResult<List<ResourceLocation>> result = ResourceLocation.CODEC.listOf().parse(JsonOps.INSTANCE, object.get("elements"));
                if(result.result().isPresent()) elements =  result.result().get().stream().map(Services.REGISTRY_UTIL::getElement).collect(Collectors.toList());
            }
