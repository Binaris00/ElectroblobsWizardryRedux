...
public class CurseOfEnfeeblement extends RaySpell {
    ...
    protected boolean onEntityHit(CastContext ctx, EntityHitResult entityHit, Vec3 origin) {
        if (entityHit.getEntity() instanceof LivingEntity target && !EBMagicDamageSource.isEntityImmune(EBDamageSources.WITHER, target)) {
            target.addEffect(new MobEffectInstance(EBMobEffects.CURSE_OF_ENFEEBLEMENT.get(), Integer.MAX_VALUE, this.property(DefaultProperties.EFFECT_STRENGTH)));
    ...
    protected @NotNull SpellProperties properties() {
        return SpellProperties.builder()
                .assignBaseProperties(Tiers.MASTER, Elements.NECROMANCY, SpellType.ALTERATION, SpellAction.POINT, 60, 20, 150)
