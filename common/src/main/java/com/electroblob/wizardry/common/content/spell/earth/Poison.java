...
public class Poison extends RaySpell {
    ...
    protected boolean onEntityHit(Level world, Entity target, Vec3 hit, @Nullable LivingEntity caster, Vec3 origin, int ticksInUse) {
        if(target instanceof LivingEntity livingTarget){
            target.hurt(livingTarget.damageSources().indirectMagic(caster, livingTarget), 5);
            ((LivingEntity) target).addEffect(new MobEffectInstance(MobEffects.POISON, (int) 200,
                    1));
        }
        return true;
    }
